...
private class DrawBackdropNode(
    ...
    private val drawBackdropLayer: DrawScope.() -> Unit = {
        ...
        if (layer != null && size.width > 0f && size.height > 0f) {
            val pad = padding
            val layerWidth = size.width.toInt() + pad.toInt() * 2
            val layerHeight = size.height.toInt() + pad.toInt() * 2
            if (layerWidth > 0 && layerHeight > 0) {
                try {
                    recordLayer(
                        effectScope,
                        layer,
                        size = IntSize(layerWidth, layerHeight),
                        block = recordBackdropBlock
                    )
                    layer.topLeft =
                        if (pad != 0f) IntOffset(-pad.toInt(), -pad.toInt())
                        else IntOffset.Zero
                    drawLayer(layer)
                } catch (_: Throwable) {
                    // Safe fallback if recording or drawing layer fails
                }
            }
        }
    }
    ...
    private fun updateEffects() {
        ...
        try {
            ...
            // Guard against shader compilation or uniform issues
        }
    }
